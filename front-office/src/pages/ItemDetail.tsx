...
import ToFavoriteToast from "../components/ToFavoriteToast";
import { z } from "zod";
...
const ItemDetail: React.FC = () => {
  ...
  const [showFavoriteToastAdd, setshowFavoriteToastAdd] =
    useState<boolean>(false);
  const [showFavoriteToastDelete, setshowFavoriteToastDelete] =
    useState<boolean>(false);
  ...
  const onSubmit = async (data: ItemFormData) => {
    ...
    try {
      const response = await axios.post(
        `http://${HOST_IP}:8080/ec-202404c/cart/add`,
        cartItem
      );
  ...
  const handleFavoriteClick = async () => {
    ...
    try {
      if (isFavorite) {
        ...
        setshowFavoriteToastDelete(true);
      } else {
        await axios.post(`http://${HOST_IP}:8080/ec-202404c/favorites/add`, {
          userId: userInfo.userid,
          itemId: item.id,
        });
        setshowFavoriteToastAdd(true);
      }
    ...
    <>
      ...
      >
        <div
          ...
          style={{ width: "80%", marginLeft: "13.5rem" }}
        >
              ...
              <div className="grid gap-4 lg:grid-cols-5">
                ...
                <div className="relative overflow-hidden rounded-lg bg-blue-gray-100 lg:col-span-4">
                  ...
                  >
                    ...
                    {showTooltip && (
                      <div className="absolute right-0 top-full mt-2 w-48 bg-gray-700 text-white text-center text-xs rounded-md py-1 px-2 shadow-lg transition-opacity duration-300 ease-in-out opacity-0 hover:opacity-100">
              ...
              <div className="md:py-8">
                <div className="mb-2 md:mb-3">
                  <h2
                    ...
                    style={{ marginTop: "4px", maxWidth: "360px" }}
                  >
                    {item.name}
                  </h2>
                </div>
                ...
                <div className="mb-8 md:mb-10">
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div
                      className="relative w-28 h-10 bg-blue-gray-900 text-white text-center rounded-md font-arial transition-colors duration-300 group overflow-hidden border border-transparent"
                      style={{ marginTop: "20px", width: "193px" }}
                    >
                ...
                <div className="mt-5">
                  ...
                  <p className="text-gray-700" style={{ maxWidth: "360px" }}>
                    {item.description}
                  </p>
                </div>
              </div>
          ...
          />
          <ToFavoriteToast
            show={showFavoriteToastAdd}
            message="お気に入りに追加しました"
            onClose={() => setshowFavoriteToastAdd(false)}
          />
          <ToFavoriteToast
            show={showFavoriteToastDelete}
            message="お気に入りから削除しました"
            onClose={() => setshowFavoriteToastDelete(false)}
          />
        </div>
      </div>
      <Footer />
